...
bool Cube::Initialize()
{
	...
	glBufferData(GL_ARRAY_BUFFER, 72*sizeof(GLfloat), _mVertices->data(), GL_STATIC_DRAW);

	glGenBuffers(1, &_mVBONormals);
	glBindBuffer(GL_ARRAY_BUFFER, _mVBONormals);
	glBufferData(GL_ARRAY_BUFFER, 72*sizeof(GLfloat), _mNormals->data(), GL_STATIC_DRAW);

	glGenBuffers(1, &_mVBOColors);
	glBindBuffer(GL_ARRAY_BUFFER, _mVBOColors);
	glBufferData(GL_ARRAY_BUFFER, 96*sizeof(GLfloat), _mColors->data(), GL_STATIC_DRAW);

	glGenBuffers(1, &_mIBOIndices);
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, _mIBOIndices);
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, 24*sizeof(GLuint), _mIndices->data(), GL_STATIC_DRAW);
...
void Cube::Draw()
{
	...
	//Create our model transformation matrices
	_mModelViewMatrix = Matrix::identityMatrix;
	_mRotationMatrix = Matrix::createRotationX(_mRotAngleX) * Matrix::createRotationY(_mRotAngleY);
